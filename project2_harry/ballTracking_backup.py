...
while True:
    ...
    # keypoints is a collection of all blobs, they are potential pool balls, so check it in a loop
    center = None
    for keypoint in keypoints:
        ...
        masked_ball_img = cv2.bitwise_and(frame, frame, mask=circle_img) # Using keypoint's info as a circle mask
        # cv2.imshow("harry previous image", masked_img)
        # masked_data = cv2.bitwise_not(masked_data, masked_data, mask=mask)
        masked_ball_hsv = cv2.cvtColor(masked_ball_img, cv2.COLOR_BGR2HSV)
        mask_ball_image_tableColor = cv2.inRange(masked_ball_hsv, table_lower, table_upper)
        mask_ball_image_tableColor = cv2.bitwise_not(mask_ball_image_tableColor)
        # Refine the ball recognition, get rid of small snippet of around pool table using color filtering again
        masked_img = cv2.bitwise_and(masked_ball_img, masked_ball_img, mask=mask_ball_image_tableColor)
        # cv2.imshow("harry final image", masked_img)
        # cv2.waitKey(0)
        masked_ball_hsv = cv2.cvtColor(masked_img, cv2.COLOR_BGR2HSV)
        # Currently only recognize write ball
        for (_ball_lower_range, _ball_upper_range) in balls_boundarie:
            # create NumPy arrays from the boundaries
            ball_lower_range = np.array(_ball_lower_range, dtype="uint8")
            ball_upper_range = np.array(_ball_upper_range, dtype="uint8")
            mask_range = cv2.inRange(masked_ball_hsv, ball_lower_range, ball_upper_range)
            ...
            if len(cnts) > 0:
                ...
                # Currently only track write ball, so once recogize it, just break the whole loop without
                # checking other pool balls
                break
    ...
    for i in range(1, len(pts)):
        ...
        print("Could draw the line for current frame!")
